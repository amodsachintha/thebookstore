...
$uidd = $_SESSION['uid'];
	...
	function checkout(){
		...
		$paidwith = $_POST['payM'];
			...
			echo "<script>window.open('gateway.php?pmeth=$paidwith&pid=$purchaseId','self')</script>";
		...
		if(isset($_POST['pmail']))
{
		...
		$r=mysqli_query($con,"select id from store.purchasehistory where purchaseId=$ptid;");
		...
		$pw=$_POST['epw'];
				...
				$que = "INSERT INTO store.pay (pid, email, pw) values ($pid,'$mail','$pw');";
		...
		if(isset($_POST['cno']))
{
		...
		$cvc=$_POST['cvc'];
				...
				$que = "INSERT INTO store.pay (pid, card, cvc) values ($pid,$cno,$cvc);";
				$run=mysqli_query($con,$que);
}



	echo "<script>alert('Purchase Successful! Thank You!')</script>";
...
$q=mysqli_query($con,"select cartId from store.cart where uid=$uid;");
...
$q2=mysqli_query($con,"select isbn,quantity from store.purchaseitems where purchaseId=$ptid;");

while($re=mysqli_fetch_array($q2))
{

$isbn=$re['isbn'];
$qun=$re['quantity'];

$i=mysqli_query($con,"insert into store.cartitems(cartID,isbn,quantity) values ($cid,$isbn,$qun);");

}


$d1=mysqli_query($con,"delete  from store.purchasehistory where id=$pid;");

$d2=mysqli_query($con,"delete from store.purchaseitems where purchaseId=$ptid;");
